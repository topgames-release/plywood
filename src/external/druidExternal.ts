...
export class DruidExternal extends External {
  ...
  constructor(parameters: ExternalValue) {
    ...
    let introspectionStrategy =
      ...
      DruidExternal.DEFAULT_INTROSPECTION_STRATEGY;
    if (
      DruidExternal.VALID_INTROSPECTION_STRATEGIES.indexOf(
        introspectionStrategy
      ) === -1
    ) {
      throw new Error(
        `invalid introspectionStrategy '${introspectionStrategy}'`
      );
    }
  ...
  public topNCompatibleSort(): boolean {
    ...
    if (refExpression instanceof RefExpression) {
      ...
      const sortApply = this.applies.find(
        (apply) => apply.name === sortRefName
      );
      if (sortApply) {
  ...
  public expressionToDimensionInflater(
    ...
    const makeExpression: () => DimensionInflater = () => {
      let druidExpression = new DruidExpressionBuilder(
        this
  ...
  public expressionToDimensionInflaterHaving(
    ...
    let dimensionInflater: DimensionInflaterHaving =
  ...
  public splitToDruid(split: SplitExpression, customOptions: any): DruidSplit {
    ...
    if (this.getQuerySelection() === "group-by-only" || split.isMultiSplit()) {
      ...
      split.mapSplits((name, expression) => {
        ...
        let { virtualColumn, dimension, inflater, having } =
    ...
    if (
      ...
      let granularityInflater = this.splitExpressionToGranularityInflater(
        splitExpression,
        label
      );
      if (granularityInflater) {
  ...
  public nestedGroupByIfNeeded(
    ...
    const parseResplitAgg = (
      applyExpression: Expression
    ): ParsedResplitAgg | null => {
  ...
  public getSubtotalsSpecQueryAndPostTransform(): any {
    const druidQuery: any = {
      queryType: "groupBy",
      dataSource: {
        type: "union",
        dataSources: [
      ...
      context: {
        timeout: 600000,
        useCache: true,
        queryId: "088c1cce-618d-4e9f-b1ea-9d79abcdac77" as any,
      },
      filter: {
        type: "and",
        fields: [
            ...
            fields: [
              {
                type: "expression",
                expression:
                  "(nvl(lookup(\"raw_platform\",'platform_group'),\"platform\")=='IOS')",
              },
              {
                type: "expression",
                expression:
                  "(nvl(lookup(\"raw_platform\",'platform_group'),\"platform\")=='Android')",
              },
            ],
          },
          {
      ...
      virtualColumns: [
        {
          ...
          name: "v:platform",
          expression:
            'nvl(lookup("raw_platform",\'platform_group\'),"platform")',
          outputType: "STRING",
        },
      ],
      dimensions: [
        ...
        },
        {
          type: "default",
          dimension: "multi_region",
          outputName: "multi_region",
        },
        {
          type: "extraction",
          dimension: "campaign_main_type",
          outputName: "lookup_campaign_main_type",
          extractionFn: {
            type: "lookup",
            retainMissingValue: true,
            lookup: {
        ...
        },
        {
          ...
          outputName: "lookup_creative_type",
          extractionFn: {
            type: "registeredLookup",
            lookup: "lane_content",
            retainMissingValue: true,
      ...
      aggregations: [
        ...
        },
        {
          ...
          aggregator: {
            ...
            fnAggregate:
              "function(current, added) { return current+(((added&1)>0)?1:0) }",
            fnCombine:
        ...
        },
        {
          ...
          aggregator: {
            ...
            fnAggregate:
              "function(current, added) { return current+(((added&32)>0)?1:0) }",
            fnCombine:
              "function(partialA, partialB) { return partialA + partialB }",
            fnReset: "function() { return 0; }",
            name: "!T_19",
  ...
  public getQueryAndPostTransform(
    ...
    // Filter
    let filterAndIntervals = new DruidFilterBuilder(this).filterToDruid(
      this.getQueryFilter()
    );
    ...
    switch (mode) {
      case "raw":
        ...
        if (this.versionBefore("0.11.0")) {
          ...
          if (!selectDimensions.length)
            selectDimensions.push(DruidExternal.DUMMY_NAME);
          if (!selectMetrics.length)
            selectMetrics.push(DruidExternal.DUMMY_NAME);
      ...
      case "value":
        ...
        if (customOptions.useSubtotalsSpec) {
          druidQuery = this.getSubtotalsSpecQueryAndPostTransform();
        }
      ...
      case "split":
        ...
        if (
          customOptions &&
          customOptions.useSubtotalsSpec &&
          druidQuery.queryType === "groupBy"
        ) {
  ...
  protected async getIntrospectAttributesWithSegmentMetadata(
    ...
    let attributes = DruidExternal.segmentMetadataPostProcess(
      timeAttribute,
      res
    );
  ...
  protected getIntrospectAttributes(
    ...
    switch (this.introspectionStrategy) {
      case "segment-metadata-fallback":
        return this.getIntrospectAttributesWithSegmentMetadata(depth).catch(
          (err: Error) => {
  ...
  protected queryBasicValueStream(
    rawQueries: any[] | null,
    customOptions: any
  ): ReadableStream {
      ...
      if (waterfallFilterExpression) {
        return External.valuePromiseToStream(
          External.buildValueFromStream(
            decomposed.external1.queryBasicValueStream(
              rawQueries,
              customOptions
            )
          ).then((pv1) => {
            ...
            return External.buildValueFromStream(
              filteredExternal.queryBasicValueStream(rawQueries, customOptions)
            ).then((pv2) => {
        ...
        let plywoodValue1Promise = External.buildValueFromStream(
          decomposed.external1.queryBasicValueStream(rawQueries, customOptions)
        );
        let plywoodValue2Promise = External.buildValueFromStream(
          decomposed.external2.queryBasicValueStream(rawQueries, customOptions)
        );
