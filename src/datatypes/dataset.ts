...
import {
  ...
  ExpressionExternalAlterationSimple,
  ExternalExpression,
  LiteralExpression,
...
interface ApplyReadyExternalsByFillAlterationAsyncTask {
  type: "external" | "expression";
  alteration: DatasetExternalAlteration;
  filledAlteration: DatasetExternalAlteration;
  datum: Datum;
  key: string;
  depth: number;
}
...
function compareItemsByKeys(item: Datum, datum: Datum, keys: string[]) {
  return keys.every(function (key) {
    // 对于时间范围对象，使用 equals 方法进行比较
    if (
      item[key] &&
      datum[key] &&
      item[key] instanceof TimeRange &&
      datum[key] instanceof TimeRange &&
      typeof (item[key] as TimeRange).equals === "function"
    ) {
      return (item[key] as TimeRange).equals(datum[key] as TimeRange);
    }
...
export class Dataset implements Instance<DatasetValue, DatasetJS> {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let attribute of attributes) {
        ...
        if (value instanceof Expression) {
          ...
          let size = sizeOfExpressionExternalAlteration(
  ...
  private addTimeRangeAttributeIfExists(
    data: Datum[],
    ...
    if (Array.isArray(data) && data.length) {
      ...
      if (firstData) {
        for (const key in firstData) {
          if (
            Object.prototype.hasOwnProperty.call(firstData, key) &&
            firstData[key] instanceof TimeRange
          ) {
            attributes.push(
              new AttributeInfo({ name: key, type: "TIME_RANGE" })
            );
            break;
          }
        }
  ...
  public applyReadyExternalsByFillAlterationSync(
    alterations: DatasetExternalAlterations,
    filledAlterations: DatasetExternalAlterations,
    splitKeys: string[]
  ): any {
    let data = this.data;
    splitKeys = splitKeys.concat(this.keys);
    for (let i = 0; i < alterations.length; i++) {
      var alteration = alterations[i];
      var filledAlteration = filledAlterations[0];
      var datum = data[alteration.index];
      var key = alteration.key;
      if (alteration.external) {
        let result = filledAlteration.result;
        let attributes = result.attributes.slice();
        let { data } = result;
        attributes = this.addTimeRangeAttributeIfExists(data, attributes);
        let newData = data.filter((item: Datum) => {
          return compareItemsByKeys(item, datum, splitKeys);
        });
        let newDataSet = {
          attributes,
          data: newData,
          keys: result.keys.slice(),
        };
        alteration.result = new Dataset(newDataSet);
      } else if (alteration.datasetAlterations) {
        (datum[key] as Dataset).applyReadyExternalsByFillAlterationSync(
          alteration.datasetAlterations,
          filledAlteration.datasetAlterations,
          splitKeys
        );
      } else if (alteration.expressionAlterations) {
        ...
        Object.keys(filledAlteration.expressionAlterations).forEach(function (
          key
        ) {
          var expressionAlteration = filledAlteration.expressionAlterations[
            key
          ] as ExpressionExternalAlterationSimple;
          if (expressionAlteration && expressionAlteration.result) {
            result = expressionAlteration.result;
          }
        });
        let { data } = result;
        let attributes = result.attributes.slice();
        ...
        Object.keys(alteration.expressionAlterations).forEach(function (key) {
          let expressionAlteration = alteration.expressionAlterations[
            key
          ] as ExpressionExternalAlterationSimple;
          expressionAlteration.result = new Dataset(newDataSet);
        });
      } else {
        throw new Error("fell through");
      }
  ...
  public async applyReadyExternalsByFillAlterationAsync(
    ...
    console.time("applyReadyExternalsByFillAlteration_total");
    ...
    // 收集所有需要处理的任务
    const collectTasks = () => {
      const tasks: ApplyReadyExternalsByFillAlterationAsyncTask[] = [];

      // 递归收集任务,传递depth参数
      const processAlteration = (
        alteration: DatasetExternalAlteration,
        filledAlteration: DatasetExternalAlteration,
        datum: Datum,
        key: string,
        ...
        if (alteration.external) {
          ...
          const subDataset = datum[key] as Dataset;
          splitKeys = splitKeys.concat(subDataset.keys);
          if (subDataset && subDataset.data) {
            alteration.datasetAlterations.forEach((subAlteration, i) => {
              const subFilledAlteration =
                filledAlteration.datasetAlterations[0];
              const subDatum = subDataset.data[subAlteration.index];
              processAlteration(
                subAlteration,
                subFilledAlteration,
                subDatum,
      ...
      for (let i = 0; i < alterations.length; i++) {
        ...
        const filledAlteration = filledAlterations[0];
    ...
    const processTask = async (
      task: ApplyReadyExternalsByFillAlterationAsyncTask
    ) => {
      console.time(`process_task_${task.type}_${task.depth}`);

      try {
        if (task.type === "external") {
          console.time("data_filter");
          const result = task.filledAlteration.result;
          const sourceData = result.data;
          const newData = sourceData.filter((item: Datum) =>
            compareItemsByKeys(item, task.datum, splitKeys)
          );
          console.timeEnd("data_filter");
          console.log("处理数据量:", sourceData.length);

          console.time("dataset_build");
          const newDataSet = {
            attributes: result.attributes.slice(),
            data: newData,
            keys: result.keys.slice(),
          };
          task.alteration.result = new Dataset(newDataSet);
          // task.datum[task.key] = newDataSet; // 设置回父级数据结构
          console.timeEnd("dataset_build");
        } else if (task.type === "expression") {
          console.time("expression_process");
          let result = new Dataset({
            attributes: [],
            data: [],
            keys: [],
          ...
          Object.keys(task.filledAlteration.expressionAlterations).forEach(
            (key) => {
              const expressionAlteration = task.filledAlteration
                .expressionAlterations[key] as DatasetExternalAlteration;
              if (expressionAlteration && expressionAlteration.result) {
                result = expressionAlteration.result;
              }
            }
          ...
          const sourceData = result.data;
          console.time("expression_data_filter");
          const newData = sourceData.filter((item) =>
            compareItemsByKeys(item, task.datum, splitKeys)
          );
          console.timeEnd("expression_data_filter");
          console.log("表达式处理数据量:", sourceData.length);
          ...
          Object.keys(task.alteration.expressionAlterations).forEach((key) => {
            const expressionAlteration = task.alteration.expressionAlterations[
              key
            ] as DatasetExternalAlteration;
            expressionAlteration.result = new Dataset(newDataSet);
          });
          // task.datum[task.key] = newDataSet;
          console.timeEnd("expression_process");
        }
      } catch (error) {
        console.error(
          `Error processing task ${task.type} at depth ${task.depth}:`,
          error
        );
        throw error;
      } finally {
        console.timeEnd(`process_task_${task.type}_${task.depth}`);
      }
    };

    // 并行处理任务
    const processTasks = async (
      tasks: ApplyReadyExternalsByFillAlterationAsyncTask[]
    ) => {
      const BATCH_SIZE = 100; // 控制并发数

      // 按深度分组处理任务
      const tasksByDepth = tasks.reduce(
        (
          acc: Record<number, ApplyReadyExternalsByFillAlterationAsyncTask[]>,
          task
        ) => {
          if (!acc[task.depth]) {
            acc[task.depth] = [];
        ...
        {} as Record<number, ApplyReadyExternalsByFillAlterationAsyncTask[]>
      );

      // 按深度顺序处理
      const depths = Object.keys(tasksByDepth).sort(
        (a, b) => Number(a) - Number(b)
      );
      for (const depth of depths) {
        const depthTasks = tasksByDepth[Number(depth)];
        console.log(`Processing ${depthTasks.length} tasks at depth ${depth}`);

        // 批量处理同一深度的任务
        for (let i = 0; i < depthTasks.length; i += BATCH_SIZE) {
          const batch = depthTasks.slice(
            i,
            Math.min(i + BATCH_SIZE, depthTasks.length)
          );
          // 直接并行处理任务，不进行重试
          await Promise.all(
            batch.map((task: ApplyReadyExternalsByFillAlterationAsyncTask) =>
              processTask(task)
            )
          );
        }
    ...
    try {
      ...
      console.log(`收集到总任务数: ${tasks.length}`);
      await processTasks(tasks);
    } catch (error) {
      console.error("处理任务失败:", error);
      throw error;
    } finally {
      console.timeEnd("applyReadyExternalsByFillAlteration_total");
    }
  }
  ...
  public findDatumByAttribute(
    ...
    return SimpleArray.find(this.data, (d) =>
      generalEqual(d[attribute], value)
    );
  }
  ...
  public flatten(options: FlattenOptions = {}): Dataset {
    ...
    let secondaryFlatAttributes: AttributeInfo[] =
